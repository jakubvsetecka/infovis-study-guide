\documentclass[11pt]{article}
\input{preamble}

\title{\vspace{-1.2cm}\textbf{\textcolor{tuwdark}{Information Visualization}}\\[2mm]
       \Large\textcolor{tuwblue}{Comprehensive Exam Study Guide}\\[2mm]
       \normalsize TU Wien, 2026S\,\textemdash\,based on the lecture slides and mandatory readings}
\author{}
\date{}

\begin{document}
\maketitle
\vspace{-1.4cm}
\begin{center}\small\itshape
This guide is organised by content domain (Sections 1--12) followed by an
exam playbook, an algorithm cheat-sheet, list-style quick references and
practice questions (Sections 13--16). Boxes mark \textcolor{tuwblue}{key points},
\textcolor{tuwdark}{algorithm fact-sheets}, \textcolor{examorange}{exam angles}
and \textcolor{pitred}{common point deductions}.
\end{center}
\vspace{2mm}
\hrule
\tableofcontents
\clearpage

\input{sections/01_intro}
\input{sections/02_data}
\input{sections/03_encoding}
\input{sections/04_perception}
\input{sections/05_design_critique}
\input{sections/06_interaction}
\input{sections/07_highdim}
\input{sections/08_scalable}
\input{sections/09_networks}
\input{sections/10_complex}
\input{sections/11_vis4ml}
\input{sections/12_evaluation}
\input{sections/13_playbook}
\input{sections/14_algo_cheatsheet}
\input{sections/15_lists}
\input{sections/16_practice}

\appendix
\input{sections/17_bridge}

\end{document}
